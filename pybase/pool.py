...
from hbase.ttypes import TIOError
    ...
    def _retry(cls, f):
        def new_f(self, *args, **kwargs):
            ...
            try:
                ...
                raise app_exc
            except (TIOError, Thrift.TException,
...
retryable = ('put', 'checkAndPut', 'get', 'openScanner',
             'deleteSingle', 'checkAndDelete')
...
class ConnectionPool(object):
    ...
    max_retries = 5
    """ When an operation on a connection fails due to an :exc:`~.TIOError`,
    the operation will be retried on different nodes up to `max_retries`
    times before an :exc:`~.MaximumRetryException` is raised.

    Setting this to 0 disables retries and setting to -1 allows unlimited
    ...
    def __init__(self,
        ...
        the pool will connect to. The port defaults to 9090 if excluded.
